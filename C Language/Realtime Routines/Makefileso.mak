...
SRCS = \
OnSys_LaserWellding.c\
usrcode.c
OBJS = $(patsubst %.c,%.o_so,$(filter %.c,$(SRCS)))

SRCS_LEVEL_0 = 
OBJS_LEVEL_0 = $(SRCS_LEVEL_0:.c.gpg=.o_so_enc_0)
OBJS_ENCRYPTED = $(OBJS_LEVEL_0)

OBJS_SHIPPED = 

# now comes a meta-rule for compiling any C source file.
$(PROG): $(OBJS) $(OBJS_SHIPPED) $(OBJS_ENCRYPTED)
	$(LD) -o $(PROG) $(OBJS) $(OBJS_SHIPPED) $(OBJS_ENCRYPTED) $(LDFLAGS) 
	if [ -d ../../Bin/DebugExp/ ]; then cp $(PROG) ../../Bin/DebugExp/usralgo.so; fi

%.o_so: %.c clean
	$(CC) $(CFLAGS) $(DTDEBUG) -c $< -o $@

%.o_so_enc_0: %.c.gpg clean
	@gpg --homedir /home/dtuser/ --batch --yes --decrypt --passphrase $(nvWHjTGlLTFPfGCU) $< 2>/dev/null | $(CC) $(CFLAGS) $(DTDEBUG) -c -o $@ -xc -
